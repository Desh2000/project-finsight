...
@router.delete(
    ...
    description="Hard deletes a transaction record by UUID. This is irreversible.",
)
def delete_transaction(txn_id: str, db: Session = Depends(get_db)):
    success, error = crud.delete_transaction(db, txn_id)
    if not success:
        raise HTTPException(status_code=404, detail=error)
    return {"message": f"Transaction '{txn_id}' has been permanently deleted."}
